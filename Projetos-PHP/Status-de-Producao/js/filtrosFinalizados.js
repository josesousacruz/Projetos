$(document).ready(function () {
    ...
    responsive: {
      details: {
        renderer: function (api, rowIdx, columns) {
          ...
          return data ? $("<table/>").append(data) : true;
        },
      },
    },
  ...
  $(document).ready(function () {
    ...
    $(document).on(
      "change",
      "#min-date, #max-date, #filterProduto, #filterEspecie",
      function () {
...
function retornaMetaDia(datachegada) {
  ...
  datachegada.forEach((element) => {
    console.log(element)

    var elemento = element.split("/").reverse().join("-");;
    var date = new Date(elemento);
    var mes = date.getMonth() + 1;
    var ano = date.getFullYear();
   
    $.post(
        "graficos/valorMetaMensal.php",
        { mes: mes, ano: ano,filtrosFinalizados:'filtrosFinalizados' },
        function (result) {
          var data = JSON.parse(result);
    
          programadoMes = data[0].valorDaMeta;

          console.log(programadoMes)
        }
      );
...
function graficoBarRelatorios(datachegada, quantidade) {
  ...
  myBarChartRelatorios = new Chart(
    document.getElementById("myBarChartRelatorios"),
    config
  );
}
...
function graficoPieRelatorio(totalProduzido) {
  ...
  myPieChartRelatorio = new Chart(
    document.getElementById("myPieChartRelatorio"),
    config
  );
}
